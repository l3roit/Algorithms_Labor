...
LinList_p LinListAllocCell(char* payload)               //-------------------------------------------DONE
{
...
void LinListFreeCell(LinList_p junk)                                //------------------------------------------DONE
{
   free(junk->payload);
   free(junk);
}
...
void LinListFree(LinList_p junk)                                    //----------------------------------DONE
{
   LinList_p next;
...
LinList_p LinListInsertFirst(LinList_p *anchor, LinList_p newcell)                  //-----------------------DONE
{
   newcell->next = *anchor;
   *anchor = newcell;
...
LinList_p LinListExtractFirst(LinList_p *anchor)                    //------------------------------DONE
{
...
void LinListPrint(FILE* out, LinList_p list)                                //-------------------------------DONE
{
   while(list)
   {
...
//
//   Split the list 'list' into two approximately equally sized lists
//   '*l1', '*l2'.
//
// Global Variables: -
//
...
void LinListSplit(LinList_p list, LinList_p *l1, LinList_p *l2)                                     //----------------------DONE
{
   LinList_p handle;
   *l1 = NULL;
   *l2 = NULL;
...
//
//   Sort a list based on its payload string. The list is sorted
//   in-place (i.e. the result list is stored in *list), but the
//   result is also returned. Uses MergeSort. 
//
// Global Variables: -
//
...
LinList_p LinListMergeSort(LinList_p *list)                         //----------------------DONE
{
